...
import numpy as np
import argparse
from train_Fourier_NN import train_Fourier_NN, train_adversarial_NN
...
parser.add_argument('-lr', '--learningrt', nargs='?', const=0.0, type=float, default=0.005, help='Static learning rate (default = 0.005)')
parser.add_argument('-a', '--alpha', nargs='?', const=0.0, type=float, default=0.0, help='Discriminator weight (default = 0.1)')
...
for ts in train_sims:
    model_name += f"_{ts}"
model_name += f"_alpha{alpha}_ws{ws}_s02"
model_dir = "models/" + model_name
'''
model_name = f"adversarial_v02_p21c_ctrpx_alpha{alpha}_lr0.003_ws{ws}_s03"
...
init_dict = None
'''
init_name = "adversarial_v03"
for ts in train_sims:
    init_name += f"_{ts}"
init_name += f"_alpha{alpha}_ws{ws}_s01"

modules = ["enc", "reg", "dis"]
init_dict = {m : f"models/{init_name}/{init_name}_{m}.pth" for m in modules}
'''
...
###
print("Training Loop")
#train_adversarial_NN(hp_model, init_dict)
