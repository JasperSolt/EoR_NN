...
from EoR_Dataset import EOREncodedImageDataset, EORImageDataset
...
class lcnn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.flatten(self.global_maxpool(x))
...
def train_lcnn(cfg):
    ...
    for t in range(epoch, cfg.model.epochs):
        ...
        # Log
        ###
        save_path = f"{path}_{t}.pth" if t%1000==0 else f"{path}.pth"
...
def predict_lcnn(cfg, mode="test"):
    ...
    test_data = EOREncodedImageDataset(mode, cfg) 
    ...
    with torch.no_grad():
        ...
        print(f"Loss: {lossfn(targets, predictions)}")
